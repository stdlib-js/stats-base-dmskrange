...
var dmskrange = tryRequire( resolve( __dirname, './../lib/dmskrange.native.js' ) );
var opts = {
...
tape( 'the function has an arity of 5', opts, function test( t ) {
	t.strictEqual( dmskrange.length, 5, 'has expected arity' );
	t.end();
});
...
tape( 'the function calculates the range of a strided array according to a mask', opts, function test( t ) {
	...
	v = dmskrange( x.length, x, 1, mask, 1 );
...
tape( 'if provided an `N` parameter less than or equal to `0`, the function returns `NaN`', opts, function test( t ) {
	...
	v = dmskrange( 0, x, 1, mask, 1 );
	t.strictEqual( isnan( v ), true, 'returns expected value' );

	v = dmskrange( -1, x, 1, mask, 1 );
	t.strictEqual( isnan( v ), true, 'returns expected value' );
